...
export const API_CONFIG = {
  ...
  // In production we try to keep requests relative so the browser uses the
  // same origin (avoiding CORS).  A build-time env var may override this
  // when necessary (e.g. during local development).
  BASE_URL: process.env.REACT_APP_API_BASE_URL || (() => {
    // if we're served from a host that matches a known tenant we use relative
    // API path.  otherwise fall back to default '/api/v1'.
    return '/api/v1';
  })(),
...
export const SERVER_CONFIG = {
  ESS_BACKEND: {
    HOST: '5.75.185.137',
    PORT: 3002,
    URL: 'http://5.75.185.137:3002'
  },
  
  // MIFOS/CBS server information (used for display only, actual
  // API calls are always routed through the ESS backend).  The backend host
  // is determined by the deployment configuration in .env (MIFOS_BASE_URL).
  MIFOS_CBS: {
    HOST: process.env.REACT_APP_MIFOS_HOST || '',
    URL: process.env.REACT_APP_MIFOS_BASE_URL || ''
  },

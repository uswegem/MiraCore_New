...
sudo tee -a /etc/nginx/sites-available/default > /dev/null << 'NGINX_CONFIG'
    ...
    # ESS API Proxy
    location /api/v1/ {
        proxy_pass http://5.75.185.137:3002/api/v1/;
    ...
    location /health {
        proxy_pass http://5.75.185.137:3002/health;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
NGINX_CONFIG
...
if curl -f http://5.75.185.137:3002/health --connect-timeout 10 > /dev/null 2>&1; then

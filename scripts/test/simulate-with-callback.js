...
const SERVER_URL = 'http://5.75.185.137:3002/api/loan';
...
async function handleCallback(response) {
    // Wait for and process callback response
    console.log('\n⏳ Waiting for callback response...');
    // In a real scenario, this would be an endpoint receiving the callback
}

async function simulateFullLoanFlow() {
    console.log('🚀 Simulating Full Loan Flow with Acknowledgment and Callbacks\n');
    ...
    try {
        ...
        const chargesResponse = await sendToServer(signedChargesRequest, 'LOAN_CHARGES_REQUEST');
        
        // Wait before next request
        await new Promise(resolve => setTimeout(resolve, 2000));

        // Step 2: LOAN_OFFER_REQUEST (with acknowledgment expectation)
        ...
        // Send offer request and expect acknowledgment
        const signedOfferRequest = digitalSignature.createSignedXML(offerRequest.Document.Data);
        const offerResponse = await sendToServer(signedOfferRequest, 'LOAN_OFFER_REQUEST');
        
        // Handle acknowledgment and wait for callback
        if (offerResponse.status === 200) {
            console.log('✅ Received acknowledgment for LOAN_OFFER_REQUEST');
            await handleCallback(offerResponse);
        }

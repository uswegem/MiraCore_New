...
(async () => {
    ...
    await simulateRequest(forwardData);
    ...
    await simulateRequest(reverseData);
    ...
    await simulateRequest(forwardLargeData);
})();
...
async function simulateRequest(data) {
    ...
    if (affordabilityType === 'FORWARD') {
        ...
        monthlyReturnAmount = await LoanCalculations.calculateEMI(eligibleAmount, interestRate, requestedTenure);
    } else {

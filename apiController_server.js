            ...
            if (loanMapping && loanMapping.mifosLoanId) {
                ...
                try {
                    ...
                    if (searchResponse.response && searchResponse.response.length > 0) {
                        const foundLoan = searchResponse.response[0];
                ...
                if (!mifosLoanId && loanNumber.startsWith('LOAN')) {
                    try {
                        // Extract timestamp from LOAN1763993570520861 -> ESS1763982075940 pattern
                        const timestamp = loanNumber.replace('LOAN', '');
                        const essAppNumber = 'ESS' + timestamp.substring(0, 13); // Take first 13 digits for ESS app pattern
                        ...
                        if (essSearchResponse.response && essSearchResponse.response.length > 0) {
                            const foundLoan = essSearchResponse.response[0];
                            mifosLoanId = foundLoan.id;
                ...
                if (!mifosLoanId) {
                    try {
                        ...
                        if (accountSearchResponse.response && accountSearchResponse.response.length > 0) {
                            const foundLoan = accountSearchResponse.response[0];
                            mifosLoanId = foundLoan.id;

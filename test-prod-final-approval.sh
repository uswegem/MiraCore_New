...
echo "��� Sending LOAN_FINAL_APPROVAL_NOTIFICATION from Production Server"
echo "=" 
...
async function sendFromProduction() {
    try {
        console.log("��� PRODUCTION SERVER CALLBACK TEST");
        ...
        console.log("��� Signing with PRODUCTION certificates...");
        const signedXML = digitalSignature.createSignedXML(callbackData.Data);
        
        const sigMatch = signedXML.match(/<Signature>(.*?)<\/Signature>/);
        if (sigMatch) {
            console.log("��� Production Signature (50 chars):", sigMatch[1].substring(0, 50) + "...");
        }

        console.log("��� Sending from production to UTUMISHI...");
        const response = await axios.post("https://gateway.ess.utumishi.go.tz/ess-loans/mvtyztwq/consume", signedXML, {
            headers: {
        ...
        if (codeMatch) {
            ...
            console.log("���️  PRODUCTION ResponseCode:", code);
            
            switch(code) {
                case "8000":
                    console.log("��� PRODUCTION SUCCESS: Signature validation PASSED!");
                    break;
                case "8009": 
                    console.log("❌ PRODUCTION FAILURE: Signature validation failed");
                    break;
                case "8011":
                    console.log("⚠️  PRODUCTION: Signature OK, business constraint");
                    break;
                default:
                    console.log("ℹ️  PRODUCTION ResponseCode:", code);
            }
        }
...
echo "��� Executing callback from production server..."

...
async function resendApprovalForESS1775372812933() {
    try {
        await mongoose.connect(process.env.MONGODB_URI);
        logger.info('✅ Connected to MongoDB');

        const LoanMapping = require('./src/models/LoanMapping');
        const loan = await LoanMapping.findOne({ essApplicationNumber: 'ESS1775372812933' });

        if (!loan) {
            logger.error('❌ Loan not found for ESS1775372812933');
        ...
        const requestedAmount = loan.requestedAmount || 579637.11;
        ...
        const messageData = {
            Data: {
                ...
                MessageDetails: {
                    ApplicationNumber: 'ESS1775372812933',
        ...
        if (response.status === 200 || response.status === 202) {
            await LoanMapping.updateOne(
                { essApplicationNumber: 'ESS1775372812933' },
                {
                    $set: {
        ...
        logger.info('========================================');
        logger.info('Application Number: ESS1775372812933');
        ...
        logger.error('❌ Error during approval submission:', {
...
// Run the function
resendApprovalForESS1775372812933();

...
**Base URL**: `http://5.75.185.137:3002/api/frontend`
...
**cURL Example**:
```bash
curl http://5.75.185.137:3002/api/frontend/health
```
...
const checkEligibility = async (nin) => {
  try {
    const response = await axios.post(
      'http://5.75.185.137:3002/api/frontend/loan/check-eligibility',
      { nin },
      {
        headers: {
...
const applyForLoan = async (loanData) => {
  try {
    const response = await axios.post(
      'http://5.75.185.137:3002/api/frontend/loan/apply',
      {
        nin: loanData.nin,
        productId: 17,
...
const checkStatus = async (appNumber) => {
  try {
    const response = await axios.get(
      `http://5.75.185.137:3002/api/frontend/loan/status/${appNumber}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`
        }
...
const LoanDetails = ({ loanNumber }) => {
  ...
  useEffect(() => {
    const fetchLoan = async () => {
      try {
        const response = await axios.get(
          `http://5.75.185.137:3002/api/frontend/loan/details/${loanNumber}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`
            }
...
const CustomerLoans = ({ nin }) => {
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      try {
        const response = await axios.get(
          `http://5.75.185.137:3002/api/frontend/customer/loans/${nin}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`
            }
...
const LoanCalculator = () => {
  ...
  const calculateLoan = async () => {
    try {
      const response = await axios.post(
        'http://5.75.185.137:3002/api/frontend/loan/calculate-schedule',
        {
          principal: amount,
          interestRate: 15.5,
...
const API_BASE_URL = 'http://5.75.185.137:3002/api/frontend';
...
# 2. Check eligibility
curl -X POST http://5.75.185.137:3002/api/frontend/loan/check-eligibility \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{"nin":"12345678901234567890"}'

# 3. Apply for loan
curl -X POST http://5.75.185.137:3002/api/frontend/loan/apply \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{
    "nin":"12345678901234567890",
...
# 4. Check status
curl http://5.75.185.137:3002/api/frontend/loan/status/ESS1734641130456 \
  -H "Authorization: Bearer YOUR_TOKEN"

# 5. Get loan details
curl http://5.75.185.137:3002/api/frontend/loan/details/ESS1734641130456 \
  -H "Authorization: Bearer YOUR_TOKEN"

# 6. Get customer loans
curl http://5.75.185.137:3002/api/frontend/customer/loans/12345678901234567890 \
  -H "Authorization: Bearer YOUR_TOKEN"

# 7. Calculate schedule
curl -X POST http://5.75.185.137:3002/api/frontend/loan/calculate-schedule \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{
    "principal":1000000,

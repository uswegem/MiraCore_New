...
TOKEN_RESPONSE=$(curl -s -X POST http://5.75.185.137:3002/api/v1/auth/login \
...
if [ -n "$TOKEN" ]; then
  ...
  curl -s -H "Authorization: Bearer $TOKEN" \
       -H "Content-Type: application/json" \
       http://5.75.185.137:3002/api/v1/loan/list-employee-loan
  ...
  RESPONSE=$(curl -s -w "%{http_code}" -o /tmp/response.txt http://5.75.185.137:3002/api/v1/loan/list-employee-loan)

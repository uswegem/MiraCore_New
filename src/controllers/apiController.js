...
const handleLoanRestructureRejection = async (parsedData, res) => {
    try {
        ...
        try {
            ...
            // If no active mapping exists, acknowledge anyway
            const responseData = {
                Data: {
                    Header: {
                        "Sender": process.env.FSP_NAME || "ZE DONE",
                    ...
                    MessageDetails: {
                        "ResponseCode": "8000",
                        "Description": `Restructure rejection acknowledged for application: ${applicationNumber}. No active loan found.`
                    }
                }
            };
            ...
            const signedResponse = digitalSignature.createSignedXML(responseData.Data);
            res.set('Content-Type', 'application/xml');
            return res.send(signedResponse);
        }
        ...
        // Check if loan is in a state where restructure can be rejected
        const allowedStatuses = ['RESTRUCTURE_PENDING', 'RESTRUCTURE_IN_PROGRESS', 'DISBURSED'];
        
        if (!allowedStatuses.includes(loanMapping.status)) {
            logger.warn('Loan restructure rejection received for unexpected status:', { 
                status: loanMapping.status,
                applicationNumber 
            });
            // Still acknowledge but log the unexpected status
        }
        ...
        const updateResult = await LoanMappingService.updateStatus(
            ...
            'RESTRUCTURE_REJECTED',
            {
                ...
                fspReferenceNumber: fspReferenceNumber || loanMapping.fspReferenceNumber,
                loanNumber: loanNumber || loanMapping.loanNumber
            }
        );

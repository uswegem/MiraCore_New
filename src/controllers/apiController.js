...
async function handleProductCreateJsonRequest(req, res) {
    try {
        ...
        if (!productCode || !productName || !deductionCode) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: productCode, productName, deductionCode'
            });
        }

        const termsArray = Array.isArray(termsCondition) ? termsCondition : [];
        const termsXML = termsArray.map((term) => {
            const effectiveDate = term?.effectiveDate || new Date().toISOString().split('T')[0];
            return `
            <TermsCondition>
                <TermsConditionNumber>${escapeXml(term?.termNumber || '')}</TermsConditionNumber>
                <Description>${escapeXml(term?.description || '')}</Description>
                <TCEffectiveDate>${escapeXml(effectiveDate)}</TCEffectiveDate>
            </TermsCondition>`;
        }).join('');

        const messageDetailsXML = `
        <MessageDetails>
            <DeductionCode>${escapeXml(deductionCode)}</DeductionCode>
            ...
            <ForExecutive>${forExecutive === true || String(forExecutive).toLowerCase() === 'true'}</ForExecutive>
            <MinimumTenure>${escapeXml(minTenure || '')}</MinimumTenure>
            <MaximumTenure>${escapeXml(maxTenure || '')}</MaximumTenure>
            <InterestRate>${escapeXml(interestRate || 0)}</InterestRate>
            <ProcessFee>${escapeXml(processingFee || 0)}</ProcessFee>
            <Insurance>${escapeXml(insurance || 0)}</Insurance>
            <MaxAmount>${escapeXml(maxAmount || 0)}</MaxAmount>
            <MinAmount>${escapeXml(minAmount || 0)}</MinAmount>
            <RepaymentType>${escapeXml(repaymentType || 'Flat')}</RepaymentType>
            <Currency>TZS</Currency>
            <InsuranceType>${escapeXml(insuranceType || 'DISTRIBUTED')}</InsuranceType>
            <ShariaFacility>${shariaFacility === true || String(shariaFacility).toLowerCase() === 'true'}</ShariaFacility>${termsXML}
        </MessageDetails>`;

        const callbackData = {
            Data: {
                Header: {
                    Sender: fspName || process.env.FSP_NAME || 'ZE DONE',
                    Receiver: 'ESS_UTUMISHI',
                    FSPCode: fspCode || process.env.FSP_CODE || 'FL8090',
                    MsgId: getMessageId('PRODUCT_DETAIL'),
                    MessageType: 'PRODUCT_DETAIL'
                },
                MessageDetails: messageDetailsXML
            }
        };

        const callbackResponse = await sendCallback(callbackData);
        const callbackPayload = callbackResponse?.data;
        const callbackPayloadStr = typeof callbackPayload === 'string'
            ? callbackPayload
            : JSON.stringify(callbackPayload || {});

        const responseCodeMatch = callbackPayloadStr.match(/<ResponseCode>(.*?)<\/ResponseCode>/i);
        const statusCodeMatch = callbackPayloadStr.match(/<StatusCode>(.*?)<\/StatusCode>/i);
        const essResponseCode = (responseCodeMatch?.[1] || statusCodeMatch?.[1] || '').trim();

        if (essResponseCode && essResponseCode !== '8000') {
            logger.error('Product create/send rejected by ESS business response', {
                essResponseCode,
                callbackHttpStatus: callbackResponse?.status,
                callbackPayload: callbackPayloadStr
            });

            return res.status(502).json({
                success: false,
                message: `ESS rejected PRODUCT_DETAIL sync (code: ${essResponseCode})`,
                data: {
        ...
        return res.status(200).json({
            success: true,
            message: 'PRODUCT_DETAIL sent successfully',
            data: {
                callbackStatus: callbackResponse?.status,
                essResponseCode: essResponseCode || '8000'
            }
        });
    } catch (error) {
        logger.error('Error handling JSON product-create request:', error);
        return res.status(500).json({
            success: false,
            message: error?.message || 'Error processing PRODUCT_DETAIL request'
        });
    }
}
    ...
    try {
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            try {
                ...
                if (req.path === '/product-create' || req.originalUrl.includes('/auth/product-create')) {
                    return await handleProductCreateJsonRequest(req, res);
                }

                return sendErrorResponse(
                    res,
                    '8001',
                    'JSON requests are only supported for /api/v1/auth/product-create',
                    'json',
                    null
                );
            } catch (parseError) {
        ...
        setImmediate(async () => {
            try {
                ...
                const loanMappingData = {
                    ...
                    tenure: messageDetails.LoanTenure || messageDetails.Tenure || LOAN_CONSTANTS.DEFAULT_TENURE,
                            ...
                            try {
                                ...
                                if (!existingClientByNin?.pageItems?.length) {
                                    ...
                                    const clientPayload = {
                                        ...
                                        ...(process.env.CBS_GENDER_MALE_ID || process.env.CBS_GENDER_FEMALE_ID ? {
                                            genderId: (clientData.Sex === 'M' || clientData.sex === 'M')
                                                ? parseInt(process.env.CBS_GENDER_MALE_ID)
                                                : parseInt(process.env.CBS_GENDER_FEMALE_ID)
                                        } : {}),
                                        ...(process.env.CBS_CLIENT_TYPE_ID ? { clientTypeId: parseInt(process.env.CBS_CLIENT_TYPE_ID) } : {}),
                                ...
                                if (clientId) {
                                    ...
                                    const loanPayload = {
                                        ...
                                        interestRatePerPeriod: parseInt(process.env.CBS_INTEREST_RATE_PER_PERIOD || '28'),
                                        ...
                                        dateFormat: "yyyy-MM-dd",
                                        locale: "en",
                                        // Disbursement charges (Account Transfer mode — auto-deducted from cash sent to customer).
                                        // Mifos requires chargeId + amount even for percentage-based charges.
                                        charges: (() => {
                                            const c = LoanCalculations.calculateCharges(parseFloat(loanAmount));
                                            return [
                                                { chargeId: 1, amount: c.processingFee.toFixed(2) },
                                                { chargeId: 2, amount: c.insurance.toFixed(2) },
                                                { chargeId: 3, amount: c.otherCharges.toFixed(2) }
                                            ];
                                        })(),
                                    ...
                                    if (loanResponse.data && loanResponse.data.loanId) {
                                        ...
                                        // Disburse loan at full principal.
                                        // Mifos automatically deducts Account Transfer charges (Processing Fee, Insurance,
                                        // Other Charges) from the cash released to the customer. The loan account is booked
                                        // at the full principal regardless.
                                        // For takeover loans, the takeOverAmount is additionally withheld for FSP1 settlement.
                                        let netDisbursementAmount = loanAmount;
                                        if (isTakeover && takeOverAmount > 0) {
                                            netDisbursementAmount = loanAmount - takeOverAmount;
                                            logger.info(`💰 Takeover loan - net to customer after FSP1 settlement: ${netDisbursementAmount}`);
                                        }

                                        const disbursePayload = {
                                            actualDisbursementDate: new Date().toISOString().split('T')[0],
                                            transactionAmount: loanAmount.toString(),
                                            dateFormat: "yyyy-MM-dd",
                                            locale: "en",
                                            note: isTakeover ? `Takeover disbursement. Net to customer after FSP1 settlement: ${netDisbursementAmount}` : undefined
                                        };

                                        await api.post(`/v1/loans/${loanId}?command=disburse`, disbursePayload);
                                        logger.info(`Loan ${loanId} disbursed at full principal: ${loanAmount}. Mifos deducts charges automatically.`);
                                ...
                                logger.error('Error in loan creation process:', {
                                    ...
                                    mifosError: error.originalError || error.response?.data,
                                    applicationNumber: messageDetails.ApplicationNumber
                                });
                                // Record error in loan mapping and stop — do NOT fall through to disbursement
                                try {
                                    await LoanMappingService.updateStatus(messageDetails.ApplicationNumber, 'FINAL_APPROVAL_RECEIVED', {
                                        errorLogs: [
                                            ...(existingMapping?.errorLogs || []),
                                            {
                                                error: error.message,
                                                mifosError: error.originalError || JSON.stringify(error.response?.data),
                                                stage: 'loan_creation',
                                    ...
                                    logger.error('Failed to update error log:', updateErr.message);
                                }
                                throw error;
                            }
                ...
                if (messageDetails.Approval === 'APPROVED') {
                    try {
                        // Calculate net disbursed amount (principal minus upfront charges)
                        const principal = savedMapping.requestedAmount;
                        const upfrontCharges = LoanCalculations.calculateCharges(principal);
                        const upfrontDeduction = upfrontCharges.processingFee + upfrontCharges.insurance + upfrontCharges.otherCharges;
                        const netDisbursed = principal - upfrontDeduction;

                        // Calculate total repayable (principal + total interest)
                        const interestRate = LOAN_CONSTANTS.DEFAULT_INTEREST_RATE;
                        const tenure = savedMapping.tenure || LOAN_CONSTANTS.DEFAULT_TENURE;
                        const totalInterest = await LoanCalculations.calculateTotalInterest(principal, interestRate, tenure);
                        const totalAmountToPay = principal + totalInterest;

                        logger.info('💰 Disbursement amounts:', {
                            principal,
                            processingFee: upfrontCharges.processingFee,
                            insurance: upfrontCharges.insurance,
                            otherCharges: upfrontCharges.otherCharges,
                            netDisbursed,
                            totalInterest,
                            totalAmountToPay
                        });
                        ...
                        logger.info('✅ Loan mapping updated with disbursement details:', {
                            ...
                            netDisbursed: updatedMapping.disbursedAmount,
                            totalAmountToPay: updatedMapping.totalAmountToPay
                        });

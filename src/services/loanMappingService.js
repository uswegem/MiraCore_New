...
class LoanMappingService {
  ...
  static async createOrUpdateWithClientData(applicationNumber, checkNumber, clientData, loanData, employmentData, originalMessageType = null) {
    try {
      ...
      const update = {
        ...
        // Promote banking/contact fields to top level for direct querying
        ...(clientData?.swiftCode && { swiftCode: clientData.swiftCode }),
        ...(clientData?.bankAccountNumber && { bankAccountNumber: clientData.bankAccountNumber }),
        ...(clientData?.mobileNumber && { mobileNumber: clientData.mobileNumber }),
        metadata: {
  ...
  static async createInitialMapping(essApplicationNumber, essCheckNumber, fspReferenceNumber, loanDetails) {
    ...
    try {
      // Validate required parameters (allow fallback to check number)
      if (!essApplicationNumber) {
        if (essCheckNumber) {
          logger.warn('ESS Application Number missing, falling back to check number');
          essApplicationNumber = essCheckNumber;
        } else {
  ...
  static async getAllWithDetails(params = {}) {
    try {
      ...
      // Batch-fetch product interest rates for all unique product codes in this page
      const productCodes = [...new Set(loans.map(l => l.productCode).filter(Boolean))];
      const products = productCodes.length
        ? await Product.find({ productCode: { $in: productCodes } }).select('productCode interestRate').lean()
        : [];
      const productRateMap = Object.fromEntries(products.map(p => [p.productCode, p.interestRate]));

      // Transform data for frontend compatibility
      const transformedLoans = loans.map(loan => {
        const clientData = loan.metadata?.clientData || null;
        return {
          _id: loan._id,
          ...
          // Annual interest rate (%) from the Product definition — the loanData.interestRate
          // field from the XML is a total TZS amount, not a percentage rate
          annualInterestRate: productRateMap[loan.productCode] ?? null,
          requestedAmount: loan.requestedAmount,
          tenure: loan.tenure,
          status: loan.status,
          // Top-level fields with fallback to metadata.clientData for older records
          swiftCode: loan.swiftCode || clientData?.swiftCode || null,
          bankAccountNumber: loan.bankAccountNumber || clientData?.bankAccountNumber || null,
          mobileNumber: loan.mobileNumber || clientData?.mobileNumber || null,

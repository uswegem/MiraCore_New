...
router.post('/sync-to-utumishi', authMiddleware, roleMiddleware(['super_admin', 'admin']), async (req, res) => {
  try {
    ...
    // ESS business-level success code is expected to be 8000
    if (essResponseCode && essResponseCode !== '8000') {
      logger.error('Product sync rejected by ESS business response', {
        essResponseCode,
    ...
    logger.info(`Product sync to Utumishi completed by ${req.user?.username} - ${products.length} products`);
    
    res.json({
      success: true,
      message: `${products.length} products sent to Utumishi`,
      data: {

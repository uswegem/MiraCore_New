...
const logMetricsError = (message, details) => {
    try {
        console.error(`[metrics] ${message}`, details || '');
    } catch (error) {
    }
};
...
const trackLoanMessage = (messageType, status = 'success') => {
    ...
    try {
        ...
        logMetricsError('Error tracking loan message:', { error: error.message, messageType, status });
    }
};
...
const trackLoanError = (errorType, messageType) => {
    ...
    try {
        ...
        logMetricsError('Error tracking loan error:', { error: error.message, errorType, messageType });
    }
};
...
const updateLoanStatusMetrics = async () => {
    try {
        ...
        logMetricsError('Error updating loan status metrics:', error);
    }
};

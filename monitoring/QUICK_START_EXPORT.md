...
curl "http://5.75.185.137:3002/api/frontend/loan/records?limit=1000"
```
...
1. **Method A: Direct Download**
   ```bash
   curl -o all-loans.csv "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000"
   # Open all-loans.csv in Excel
   ```

2. **Method B: Power Query in Excel**
   - Open Excel → Data → Get Data → From Web
   - URL: `http://5.75.185.137:3002/api/frontend/loan/records?limit=5000`
   - Click OK → Load
...
curl "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&limit=500&format=csv"

# Get next 500 (pagination)
curl "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&limit=500&offset=500&format=csv"
```
...
Invoke-WebRequest -Uri "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000" -OutFile "$env:USERPROFILE\Downloads\loans.csv"

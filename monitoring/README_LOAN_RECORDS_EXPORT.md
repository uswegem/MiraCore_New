...
```bash
# Rejected loans
curl -o rejected-loans.csv "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&format=csv&limit=5000"

# Disbursed loans
curl -o disbursed-loans.csv "http://5.75.185.137:3002/api/frontend/loan/records?status=DISBURSED&format=csv&limit=5000"
```
...
function importLoans() {
  var url = 'http://5.75.185.137:3002/api/frontend/loan/records?limit=1000';
...
```bash
# Get all loans (JSON)
curl http://5.75.185.137:3002/api/frontend/loan/records

# Get rejected loans (JSON)
curl http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED

# Get first 100 disbursed loans (CSV)
curl http://5.75.185.137:3002/api/frontend/loan/records?status=DISBURSED&limit=100&format=csv

# Pagination - get next 1000 records
curl http://5.75.185.137:3002/api/frontend/loan/records?limit=1000&offset=1000
```
...
curl -o disbursed-loans.csv \
  "http://5.75.185.137:3002/api/frontend/loan/records?status=DISBURSED&format=csv&limit=5000"
```
...
0 8 * * * curl -o /home/reports/loans-$(date +\%Y\%m\%d).csv "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000"
```

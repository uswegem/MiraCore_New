...
1. **Download the latest dashboard**: 
   ...
   - Or download from: http://5.75.185.137:3002/ (if you have file access)
...
2. **Configure First Panel (All Loans)**:
   - **Query**:
     ...
     - **Source**: URL
     - **Method**: GET
     - **URL**: `http://5.75.185.137:3002/api/frontend/loan/records?limit=1000`
     - **Format**: Table
     - **Root/Rows**: `data`
...
```bash
# Test JSON response
curl http://5.75.185.137:3002/api/frontend/loan/records?limit=2
...
docker exec grafana curl -s http://5.75.185.137:3002/api/frontend/loan/records?limit=1
...
1. **Check API is running**:
   ```bash
   curl http://5.75.185.137:3002/api/frontend/loan/records?limit=1
   ```

2. **Check from Grafana container**:
   ```bash
   ssh uswege@5.75.185.137 'docker exec grafana curl -s http://5.75.185.137:3002/api/frontend/loan/records?limit=1'
   ```
...
ssh uswege@5.75.185.137 'netstat -tlnp | grep 3002'
...
ssh uswege@5.75.185.137 'sudo ufw allow 3002/tcp'
```
...
- [ ] API endpoint accessible: `curl http://5.75.185.137:3002/api/frontend/loan/records?limit=1`
- [ ] API accessible from Grafana: `docker exec grafana curl http://5.75.185.137:3002/api/frontend/loan/records?limit=1`
...
4. Query type: JSON
5. URL: http://5.75.185.137:3002/api/frontend/loan/records?limit=5
6. Root: data
7. Format: Table
8. Apply
```

...
### Download All Loans
```bash
curl -o loans.csv "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000"
```

### Download by Status
```bash
# Rejected loans
curl -o rejected.csv "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&format=csv"

# Disbursed loans
curl -o disbursed.csv "http://5.75.185.137:3002/api/frontend/loan/records?status=DISBURSED&format=csv"

# All other statuses
curl -o initial.csv "http://5.75.185.137:3002/api/frontend/loan/records?status=INITIAL_OFFER&format=csv"
```
...
Invoke-WebRequest -Uri "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000" -OutFile $file
...
```
GET http://5.75.185.137:3002/api/frontend/loan/records
```
...
```bash
# All loans
curl "http://5.75.185.137:3002/api/frontend/loan/records?limit=100"

# Filtered by status
curl "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&limit=100"

# With pagination
curl "http://5.75.185.137:3002/api/frontend/loan/records?limit=100&offset=100"
```

**CSV (for Excel/reporting):**
```bash
# Direct download
curl -o output.csv "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000"
...
curl -o "rejected-$DATE.csv" \
  "http://5.75.185.137:3002/api/frontend/loan/records?status=REJECTED&format=csv&limit=5000"
```

### 2. Excel Power Query Auto-Refresh
1. Open Excel
2. **Data** → **Get Data** → **From Web**
3. URL: `http://5.75.185.137:3002/api/frontend/loan/records?limit=1000`
4. Click **OK** → **Load**
5. Right-click table → **Refresh** to update

### 3. Power BI Integration
1. **Get Data** → **Web**
2. URL: `http://5.75.185.137:3002/api/frontend/loan/records?limit=5000`
3. **Advanced** → **OK**
...
$url = "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5000"
...
```bash
curl -s "http://5.75.185.137:3002/api/frontend/loan/records?limit=1" | head -20
```

### Test CSV Export
```bash
curl -s "http://5.75.185.137:3002/api/frontend/loan/records?format=csv&limit=5" | head -3
```
...
1. Check API: `curl http://5.75.185.137:3002/api/frontend/loan/records?limit=1`
